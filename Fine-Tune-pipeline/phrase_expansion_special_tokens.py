...
class ModelTrainer_notebook:
    def __init__(
        ...
        self.run_name = f"{self.model_name}_ep-{self.num_epochs}_ga-{self.gradient_accumulation_steps}_b-{self.batch_size}_lr-{self.learning_rate}_wd-{self.weight_decay}"
        self.output_dir = (
            f"/home/user/ksharma/ks_thesis/saved_models/final_models/{self.run_name}"
        )
    ...
    def preprocess_function(self, data):
        ...
        inputs = [
            input_text.replace("<SoftSkill_C>", "<SS_C>")
            .replace("</SoftSkill_C>", "</SS_C>")
            .replace("<SoftSkill>", "<SS>")
            .replace("</SoftSkill>", "</SS>")
    ...
    def train(self):
        ...
        args = Seq2SeqTrainingArguments(
            # output_dir = f"{self.model_name}-finetuned-noun_ellipse_rmt_ver3",
            # output_dir = self.output_dir,
            # output_dir = f"{self.output_dir}-saved",
            output_dir=self.output_dir,
            # f"en-{self.model_name}-finetuned-noun_ellipse_rmt_ver2",
        ...
        # # Wrap the optimizer with GradientAccumulationScheduler
        # optimizer = trainer.optimizer
        # scheduler = GradientAccumulationScheduler(optimizer, gradient_accumulation_steps)
    ...
    def save(self):
        ...
        self.model.save_pretrained(
            f"/home/user/ksharma/ks_thesis/saved_models/final_models/{self.run_name}_special_token_{timestamp}"
        )
    ...
    def test(self):
        ...
        testing_df.to_csv(
            f"/home/user/ksharma/ks_thesis/saved_data_files/updated_self_model/{self.run_name}_{timestamp}.csv",
            index=True,
        )
...
def main(
    ...
    special_tokens = ["<SS_C>", "</SS_C>", "<SS>", "</SS>"]
    tokenizer.add_tokens(special_tokens)
    model.resize_token_embeddings(len(tokenizer))

    model.config.vocab_size = len(tokenizer)
    model.config.max_position_embeddings = len(tokenizer)
